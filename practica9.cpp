...
float movCoche;
float movOffset;
float toffsetnumerovCambio = 0.0;
float rotllanta;
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Instancia del humo 2
		color = glm::vec3(0.7f, 0.7f, 0.7f);
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(4.0f, 10.0f + exp(exponencial), 4.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		...
		//Instancia del humo 3
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(6.0f, 10.0f + exp(exponencial), 6.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.001f, 0.001f, 0.001f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Humo_M.RenderModel();

		//Instancia del humo 4
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(4.0f, 10.0f + exp(exponencial), 6.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.001f, 0.001f, 0.001f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Humo_M.RenderModel();

		//Instancia del humo 5
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(6.0f, 10.0f + exp(exponencial), 4.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.001f, 0.001f, 0.001f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		*/
		...
		case 0:
			...
			toffsetnumerovCambio = -0.66;
			break;
		case 1:
			toffsetnumerocambiau = 0.0;
			toffsetnumerovCambio = 0.0;
			break;
		...
		case 3:
			toffsetnumerocambiau = 0.5;
			toffsetnumerovCambio = 0.0;
			break;

		case 4:
			toffsetnumerocambiau = 0.75;
			toffsetnumerovCambio = 0.0;
			break;

		case 5:
			toffsetnumerocambiau = 0.0;
			toffsetnumerovCambio = -0.33;
			break;
		...
		if (contadorCambio % 100 == 0 && contadorCambio != 0) {
			//printf("Cambia %d\n", cambiaUnidades);
			cambiaUnidades++;
			
		}
		
		if (cambiaUnidades == 10) cambiaUnidades = 0;
		
		
		toffsetnumerov = toffsetnumerovCambio;
		...
		//if
		if (contadorCambio % 1000 == 0 && contadorCambio != 0) {
			cambiaDecenas++;
		}
		if (cambiaDecenas == 10) {
			cambiaDecenas = 0;
		...
		if (exponencial >= 20) exponencial = 0;
